...
fun CreateAccountScreen(
    ...
    navigateToClientAccountCreation:() -> Unit,
    navigateToFreelancerAccountCreation:() -> Unit,
    viewModel: AuthViewModel = hiltViewModel()
) {
    val customerType = viewModel.customerType.collectAsState().value

    var email by remember { mutableStateOf("") }
    var password by remember { mutableStateOf("") }
    val signUpModel = SignUpModel(customerType, email, password)
    ...
    when (uiState) {
        ...
        is AuthUiState.successState -> {
            ...
            if (customerType == "client"){
                navigateToClientAccountCreation.invoke()
            } else {
                navigateToFreelancerAccountCreation.invoke()
            }
        }
        else -> {}
    }
